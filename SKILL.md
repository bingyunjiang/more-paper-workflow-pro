...
| 脚本 | 步骤 | 用途 |
|------|------|------|
| `scripts/search_by_topic.py` | 4 | 多渠道检索（Semantic Scholar / Crossref / OpenAlex / 🆕 Wanfang Data）+ 引文网络 + 语义缓存。Wanfang 中文文献：校内 IP 直连零配置；校外需 CDP Chrome + CARSI SSO（`start_cdp_chrome.sh` → 浏览器登录 `fsso.wanfangdata.com.cn`）|
