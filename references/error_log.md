...
- **Cause:** OpenAlex API 返回的 `primary_location.source` 可能为 `None`，原链式调用 `(p.get("primary_location") or {}).get("source", {}).get("display_name", "?")` 在 `source=None` 时崩溃
...
- **Cause:** `search=` 参数匹配全文 + 元数据，缺乏 title 级精度过滤。宽泛查询如 "EV charger thermal management" 会匹配到无关领域的 thermal/cooling 论文
- **Correction rule:** 始终添加 `filter=type:article` + `title.search:` 子句。对于传统工科方向，优先使用 3 个 `AND` 块的 title-focused 查询而非全文搜索。当检索结果噪声 > 80% 时，改用直接 OpenAlex API 调用 + 手动 title 关键词过滤。
- **Impact:** 中 — 增加人工筛选工作量
